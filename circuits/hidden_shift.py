...
def run(min_qubits=2, max_qubits=8, max_circuits=3, num_shots=1000,
    ...
    qr_state = [cirq.GridQubit(i, 0) for i in
                range(Uf_.num_qubits)]  # we need to create registers to print circuits in cirq
    print("\nQuantum Oracle 'Uf' =")
    print(cirq.Circuit(cirq.decompose(Uf_.on(*qr_state))) if Uf_ != None else " ... too large!")
    print("\nQuantum Oracle 'Ug' =")
    print(cirq.Circuit(cirq.decompose(Ug_.on(*qr_state))) if Ug_ != None else " ... too large!")
