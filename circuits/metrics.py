    ...
    if group in group_metrics["groups"]:
        ...
        if group_index >= 0:
            ...
            if len(group_metrics["avg_tr_depths"]) > 0:
                ...
                if avg_tr_depth > 0:
                    print(
                        f"Average Transpiled Depth, \u03BE (xi) for the {group} qubit group = {int(avg_tr_depth)}, {avg_tr_xi}")
            ...
            if len(group_metrics["avg_exec_creating_times"]) > 0:
                ...
                print(
                    f"Average Transpiling, Validating, Running Times for group {group} = {avg_exec_creating_time}, {avg_exec_validating_time}, {avg_exec_running_time} secs")
...
def plot_metrics(suptitle="Circuit Width (Number of Qubits)", transform_qubit_group=False, new_qubit_group=None,
                 filters=None, suffix=""):
...
def plot_all_app_metrics(backend_id, do_all_plots=False,
    ...
    if do_volumetric_plots:
        ...
        cmap = cmap_spectral
        suptitle = f"Volumetric Positioning - All Applications (Merged)\nDevice={backend_id}  {timestr} UTC"
        plot_metrics_all_merged(shared_data, backend_id, suptitle=suptitle, imagename="_ALL-vplot-2" + suffix,
                                avail_qubits=avail_qubits)
