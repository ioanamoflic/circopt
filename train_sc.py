    ...
    for ep in episodes:
        for p in partition_size:
            for it in max_iter:
                ...
                env = CircuitEnvIdent(starting_circuit, filename, p)
                agent = QAgentSingleCore(env, n_ep=ep, max_iter=it, lr=0.01, gamma=0.97, expl_decay=0.001)
                agent.train()

                #agent.show_evolution(filename=filename, ep=ep)
                #filename = f'test.csv'
